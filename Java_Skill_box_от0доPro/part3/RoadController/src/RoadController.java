...
public class RoadController {
    ...
    public static void setPassengerCarMaxHeight(int passengerCarMaxWeight) {
        RoadController.passengerCarMaxWeight = passengerCarMaxWeight;
    }
    ...
    private static int calculatePrice(Car car) {
        ...
        int price = 0;
        if (carHeight > getControllerMaxHeight()) {
            blockWay("высота вашего ТС превышает высоту пропускного пункта!");
            return -1;
        } else if (carHeight > getPassengerCarMaxHeight()) {
            double weight = car.getWeight();
            //Грузовой автомобиль
            if (weight > getPassengerCarMaxWeight()) {
                price = getPassengerCarPrice();
                if (car.isHasVehicle()) {
                    price = price + getVehicleAdditionalPrice();
                }
            }
            //Легковой автомобиль
            else {
                price = getCargoCarPrice();
            }
        } else {
            price = getPassengerCarPrice();
        }
